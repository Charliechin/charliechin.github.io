...
FOLDERS = {
    "_assets/images/birras/": [1230, 1024, 874, 655, 560],
    "_assets/images/": []  # Do not make any additional sizes
}
...
staged_files = [x.a_blob.path for x in diffs if hasattr(
    x, 'a_blob') and x.a_blob != None]
...
for folder in FOLDERS.keys():
    ...
    for image in realimages:
        ...
        width = int(subprocess.check_output(
            "identify -format \"%[w]\" " + imagepath, shell=True))
            ...
            if image.endswith(".jpg") or image.endswith(".jpeg"):
                ...
                for size in FOLDERS[folder]:
                    ...
                    if size > width:
                        ...
                        os.system(
                            "convert " + imagepath + " -sampling-factor 4:2:0 -strip -quality 85 -interlace JPEG -colorspace RGB " + newsizeimagepath)
                    else:
                        os.system("convert " + imagepath + " -sampling-factor 4:2:0 -strip -resize " + str(
                            size) + "x -quality 85 -interlace JPEG -colorspace RGB " + newsizeimagepath)
                    # Add the resized image
                    images.append(newsizeimage)
                    repo.git.add(newsizeimagepath)
                # Optimise the original
                os.system("convert " + imagepath +
                          " -sampling-factor 4:2:0 -strip -quality 85 -interlace JPEG -colorspace RGB " + imagepath)
                ...
                for size in FOLDERS[folder]:
                    ...
                    if size > width:
                        ...
                        # Make it smaller
                        os.system("convert " + imagepath + " -strip -resize " +
                                  str(size) + "x " + newsizeimagepath)
                        # Also optimise it
                        os.system("optipng -quiet -o1 -strip all " +
                                  newsizeimagepath)
                    # Add the resized image
                    images.append(newsizeimage)
                    repo.git.add(newsizeimagepath)
